...
export function StatsCard({ stats, isLoading, onStartReview }: StatsCardProps) {
  ...
  if (!stats) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        <p className="text-gray-500 dark:text-gray-400">{t('vocabulary.stats.loadError')}</p>
      </div>
    );
  }
  ...
  const statItems = [
    {
      label: t('vocabulary.stats.total'),
      value: stats.total,
      color: 'bg-gray-500',
      bgColor: 'bg-gray-50 dark:bg-gray-700/50',
    },
    {
      label: t('vocabulary.stats.new'),
      value: stats.new,
      color: 'bg-blue-500',
      bgColor: 'bg-blue-50 dark:bg-blue-900/20',
    },
    {
      label: t('vocabulary.stats.learning'),
      value: stats.learning,
      color: 'bg-orange-500',
      bgColor: 'bg-orange-50 dark:bg-orange-900/20',
    },
    {
      label: t('vocabulary.stats.mastered'),
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400">{t('vocabulary.stats.dueTodayLabel')}</p>
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              {t('vocabulary.stats.dueTodayCount', { count: stats.dueToday })}
            </p>
          </div>
          {stats.dueToday > 0 && (
        ...
        {stats.dueToday > 0 && onStartReview && (
          ...
          >
            {t('vocabulary.stats.startReview')}
          </button>
        )}
